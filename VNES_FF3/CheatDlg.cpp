...
//	DEBUGOUT( "CCheatCodeDlg::OnLoad\n" );
	...
	if( Config.path.bCheatPath ) {
		...
		tempstr = CPathlib::MakePathExt( Config.path.szCheatPath, Emu.GetNES()->rom->GetRomName(), "vct" );
	} else {
		tempstr = CPathlib::MakePathExt( Emu.GetNES()->rom->GetRomPath(), Emu.GetNES()->rom->GetRomName(), "vct" );
	}
//	DEBUGOUT( "Path: %s\n", tempstr.c_str() );
